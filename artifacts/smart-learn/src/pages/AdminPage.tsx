...
function Dashboard({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  const { data: plugins = [], isLoading, error } = useQuery<DbPlugin[]>({
    ...
    <div style={{ minHeight: "100vh", background: "#07070f" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "6rem 1.5rem 3rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 32 }}>
          <div>
            <h1 style={{ fontSize: 28, fontWeight: 900, color: "#fff", margin: "0 0 6px" }}>Plugin Manager</h1>
            <p style={{ fontSize: 14, color: "#64748b" }}>Manage all plugins in the Smart Learn database</p>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 28 }}>
          {[
            ["Total Plugins", plugins.length, "#a855f7"],
